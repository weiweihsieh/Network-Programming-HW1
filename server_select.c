...
int main()
{
	...
	char tmp[MAXLINE], message[MAXLINE], buffer[MAXLINE], clen[MAXCLEN], htmlfile[MAXLINE],c;
	FILE *fp;
	int image[100000];	
	...
	for( ; ; )
	{
		...
		if(FD_ISSET(listenfd, &readset)) //test whether fd in the set 
		{
